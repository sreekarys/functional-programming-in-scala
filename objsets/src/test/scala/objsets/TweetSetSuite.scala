...
class TweetSetSuite {
  ...
  @Test def `filter: e on set5`: Unit =
    new TestSets {
      assertEquals(0, size(set5.filter(tw => tw.user == "e")))
    }
  ...
  @Test def `mostRetweeted on empty set`: Unit = {
    new TestSets {
      try{
        set1.mostRetweeted
        Assert.fail("Empty.mostRetweeted is not defined")
      } catch{
  ...
  @Test def `mostRetweeted on set5`: Unit = {
    new TestSets {
      ...
      Assert.assertEquals(20, tweet.retweets)
      Assert.assertTrue("a".equals(tweet.user) || "b".equals(tweet.user))
    }
  }
