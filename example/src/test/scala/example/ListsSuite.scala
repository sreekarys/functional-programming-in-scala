 ...
 */
 class ListsSuite {
  ...
  @Test def `sum with negative numbers`: Unit = {
    Assert.assertEquals(sum(List(1,-1,-1)), -1)
    Assert.assertEquals(sum(List(1,-1,2,-2)), 0)
  }

  @Test def `sum on Empty List`: Unit = {
    Assert.assertEquals(sum(List()), 0)
  }

  @Test def `max on Empty List`: Unit = {
    try{
      max(List())
      Assert.fail("max method on Empty List fails!!!")
    } catch {
      case e: NoSuchElementException => ()
    }
  }

  @Test def `max of List with one element`: Unit = {
    Assert.assertEquals(max(List(5)), 5)
  }

  @Test def `max of equal numbers`: Unit = {
    Assert.assertEquals(max(List(5,5,5)), 5)
  }
